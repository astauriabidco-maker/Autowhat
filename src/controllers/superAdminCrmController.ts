...
export const sendRelanceEmail = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const templates: Record<string, { subject: string; body: string }> = {
            help: {
                subject: `${tenant.name} - Besoin d'aide pour configurer WhatsPoint ?`,
                body: `Bonjour ${admin.name},\n\nJe suis le fondateur de WhatsPoint et je vois que vous venez de créer un compte.\n\nAvez-vous besoin d'aide pour configurer votre espace ? Je suis disponible pour un appel de 15 minutes si vous le souhaitez.\n\nÀ très vite,\nL'équipe WhatsPoint`
            },
            expiring: {
                subject: `Plus que 24h pour ${tenant.name} !`,
                body: `Bonjour ${admin.name},\n\nVotre période d'essai expire demain !\n\nNe perdez pas vos données de pointage. Passez à un plan payant dès maintenant pour continuer à utiliser WhatsPoint.\n\n→ Accédez à votre espace: https://app.whatspoint.app/billing\n\nDes questions ? Répondez à cet email.\n\nL'équipe WhatsPoint`
            },
            extension: {
